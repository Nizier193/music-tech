# MusicTech Landing — `musictech.tech`
...
Приложение откроет `https://musictech.tech/login`, дождётся диплинка
...
1. **Зарегистрируйся на Resend** → https://resend.com
   ...
   - когда подключишь домен `musictech.tech` — добавь его в Resend → Domains,
     пропиши SPF/DKIM записи в DNS и поменяй `RESEND_FROM_EMAIL` на
     `noreply@musictech.tech`.
...
5. **Подключи домен `musictech.tech`:**
   - в Vercel Project Settings → Domains → Add → `musictech.tech`;
   - Vercel выдаст DNS-записи (A или CNAME), пропиши их у регистратора;
   - дождись verification (обычно 5–30 минут);
   - обнови `APP_URL=https://musictech.tech` в Environment Variables;
   - в Resend → Domains → Add Domain → `musictech.tech` → пропиши SPF/DKIM/DMARC;
   - после Verified обнови `RESEND_FROM_EMAIL=noreply@musictech.tech`.
...
не уйдёт. После подключения своего домена `musictech.tech` это ограничение
снимется.
