...
списка и заполнить таблицу `competitors-test-results.md` в этой же папке.
...
- **Алгоритм:** иерархическая HMM с явной моделью длительности
  (Cont 2010 — статья [7] в [`literature.md`](literature.md)).
...
`competitors-test-results.md` (в этой же папке):
