...
% colorlinks=false: coloured boxes around links, text unchanged
...
\author{\firstname{Н.~И.}~\surname{Новицкий}}
...
\author{\firstname{А.~А.}~\surname{Варламов}}
\email{arceniyvarlamov@gmail.com}
\affiliation{\textit{АНО Центральный Университет}}
...
пять известных в литературе применений обучения с подкреплением к
музыкальному аккомпанементу. Сформулирована теоретическая
постановка гибридного подхода, в котором агент обучения с
подкреплением действует как тонкий слой опережающего предсказания
темпа поверх классического вероятностного трекера, а не заменяет
его целиком. Численные эксперименты не приводятся; работа имеет
обзорный и теоретический характер.
\end{abstract}
...
%
Из анализа существующих работ~\cite{Dorfer2018RL,Peter2023,
Jiang2020RLDuet,Wu2024ReaLchords,Jaques2017SeqTutor} следует, что
чистая замена классического трекера агентом обучения с
подкреплением~\cite{Dorfer2018RL} требует тщательно размеченных
данных и не использует априорную структуру партитуры (повторы,
скачки, явные длительности нот). С другой стороны, классические
трекеры~\cite{Cont2010} обладают зрелой математической базой, но
их механизм опережающего предсказания темпа зашит в виде явных
байесовских обновлений и не адаптируется к стилю конкретного
солиста. Предлагается \textit{гибридная} архитектура (РИС.~1):
классический вероятностный трекер сохраняется как ядро системы, а
рядом с ним работает лёгкий агент~--- он смотрит на текущее
состояние трекера и историю темпа и предсказывает, с каким
темповым коэффициентом $a_t$ нужно играть оркестр в ближайшие
$200$--$500$\,мс. Рендерер не ждёт текущего момента~--- он сразу
применяет предсказание к ускорению или замедлению оркестрового
трека.

Состояние агента $s_t = (\hat\alpha_t,\,\tau_{t-K:t},\,
e_{t-K:t},\,\hat\varphi(t)/N)$. Здесь $\hat\alpha_t$~--- сжатое
апостериорное распределение трекера по позициям партитуры
(показывает, насколько модель уверена в том, где сейчас
исполнитель); $\tau_{t-K:t}$~--- последние $K$ оценок темпа за
прошлые $200$\,мс; $e_{t-K:t}$~--- последние $K$ значений
эмиссионной ошибки, то есть насколько реальный звук отличается от
ожидаемого по партитуре; $\hat\varphi(t)/N$~--- текущее положение
в партитуре в относительных единицах. Действие
$a_t \in [0{,}5;\,2{,}0]$~--- темповый коэффициент: значение $1{,}0$
означает номинальный темп, $1{,}2$~--- на 20\% быстрее,
$0{,}8$~--- на 20\% медленнее.

Агент обучается минимизировать вознаграждение вида
\begin{equation}
r_t = -\underbrace{|t^{\text{render}}_t - t^{\text{perf}}_t|}_{\text{рассинхронизация}}
      - \lambda\underbrace{\mathcal{L}_{\text{align}}(t)}_{\text{ошибка трекера}}
      - \mu\underbrace{(a_t - a_{t-1})^2}_{\text{резкий перепад}}
\eqlab{reward-thesis}
\end{equation}
Первая часть~--- разница между моментом, когда рендерер
...
рассогласование, тем синхроннее звучит ансамбль. Вторая~---
$\mathcal{L}_{\text{align}}$: ошибка выравнивания самого
трекера~--- гарантирует, что предсказание темпа не противоречит
тому, где по мнению трекера сейчас находится исполнитель.
Третья~--- $(a_t - a_{t-1})^2$: штраф за резкие перепады темпа,
чтобы оркестр не дёргался. Агент обучается двустадийно: сначала
на записях реальных исполнений, затем дообучается методом
проксимальной оптимизации политики~\cite{Schulman2017PPO} в
симуляторе солиста.
...
классического трекера.

Стоит специально оговорить, почему для предсказания темпа выбрано
обучение с подкреплением, а не более простая эвристика вида
«смотрим на последние $n$ наблюдений и реагируем: ускоряется
пианист~--- ускоряем оркестр; уверенность трекера упала~--- не
меняем темп резко; случился скачок позиции~--- откатываемся к
консервативному темпу». Такой подход прост, но имеет три
принципиальных ограничения. \textbf{Первое:} каждое правило
обрабатывает одну ситуацию независимо, хотя реальное исполнение
сочетает сигналы одновременно~--- ускорение при упавшей
уверенности трекера требует совсем иной реакции, чем ускорение
при высокой. \textbf{Второе:} пороги и веса подбираются вручную
и не переносятся на нового исполнителя~--- рисунок rubato у
студента и у концертирующего пианиста совершенно разный.
\textbf{Третье:} эвристика реагирует на прошлое и не учится
предсказывать. RL-агент, напротив, видит сразу весь вектор
$s_t$~--- историю темпа, уверенность трекера, место в
партитуре~--- и вырабатывает \textit{совместный} отклик на
комбинацию сигналов. Главное: тот же агент можно дообучить под
конкретного солиста без переписывания правил.

Ожидаемые свойства предложенной рамки: вероятностная
интерпретируемость через forward-распределение HMM, возможность
дообучения под конкретного солиста без изменения трекера,
и независимая диагностика каждого из двух слоёв.
